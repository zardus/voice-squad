...
List active projects. Projects are created and managed by the human via the web UI — you do not start or stop projects.
...
Create a worker in a project. Validates the task file exists and starts the agent with the task prompt.
...
Archive a completed worker. Reads a summary from stdin, captures the full pane log, moves the task file to archived, and kills the worker.
