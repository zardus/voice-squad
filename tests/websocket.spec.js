...
test.describe("WebSocket", () => {
  ...
  test("text_command returns deprecation error", async ({ page }) => {
    await page.goto(pageUrl());
        ...
        ws.onmessage = (evt) => {
          if (typeof evt.data === "string") {
            ...
            if (m.type === "connected" && !sentCommand) {
              ...
              setTimeout(() => { ws.close(); resolve("no-error"); }, 2000);
    ...
    // text_command is deprecated — hub returns an error directing clients to use pane_send_text
